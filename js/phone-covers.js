// ===== PHONE COVERS PAGE JS =====

let filteredProducts = [...phoneCovers];
...
function applyFilters() {
    ...
    if (selectedModels.length > 0) {
        results = results.filter(p => p.models.some(m => selectedModels.includes(m)));
    }
...
function clearFilters() {
    ...
    document.querySelectorAll('input[name="type"]:checked').forEach(cb => cb.checked = false);
    
    filteredProducts = [...phoneCovers];
    renderProducts(filteredProducts);
}

function quickAddToCart(productId) {
    const product = phoneCovers.find(p => p.id === productId);
    if (product) {
